...
def oss():
    ...
    img_key = "face_reognition/"+"test1.mp4"
    img_path = videosrc+"/output1.mp4"
...
def load_next_landmark(vfile, curr_frame):
    ...
    face_points = [1, 2, 3, 4, 49, 50, 51, 52, 53, 54, 62, 63, 64, 55, 14, 15,
...
if len(sys.argv) != 8:
    print('usage: python ', sys.argv[0],
          'in.jpg in.mp4 out.mp4 in.txt out.txt path audio.mp3')
    exit(0)
...
videoWriter1 = cv2.VideoWriter(sys.argv[3], fourcc, fps, (ow, oh), True)
if not videoWriter1.isOpened():
    print('无法打开视频文件', sys.argv[3])
    exit(0)
...
def change_face_main():
    ...
    command2 = "ffmpeg -i "+videosrc+"/out1.mp4"+" -i "+audio_src+" -y -c:v copy -c:a aac -strict experimental "+videosrc+"/output1.mp4"
    print(command2)
    os.system(command2)
    oss()
    sys.exit(0)
